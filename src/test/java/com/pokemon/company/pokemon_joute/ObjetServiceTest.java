...
public class ObjetServiceTest {
    ...
    public void testAcheterObjet_Success() {
        ...
        dresseur.setDresseurObjets(new ArrayList<>()); // Initialisation de la liste des liens dresseur-objet de ce dresseur
        ...
        objet.setPrix(150);
        objet.setTypeObjet(TypeObjet.POKEBALL);
        objet = objetRepository.save(objet); //objetRepository.save(objet);

        // Acheter l'objet
        String resultat = objetService.acheter(dresseur.getId(), objet.getId());
        ...
        Assertions.assertNotNull(dresseurApresAchat.getDresseurObjets());
        Assertions.assertEquals(1, dresseurApresAchat.getDresseurObjets().size()); // Vérifier que l'objet a été ajouté à l'inventaire
        Assertions.assertEquals(1, dresseurApresAchat.getDresseurObjets().get(0).getQuantite()); // Vérifier la quantité
    }
    ...
    public void testAcheterObjet_NotEnoughMoney() {
        ...
        objet.setTypeObjet(TypeObjet.DIVERS);
        ...
        Assertions.assertTrue(dresseurApresTentative.getDresseurObjets().isEmpty()); // Vérifier que l'inventaire est vide
    }
    ...
    public void testAcheterObjet_DresseurOrObjetNotFound() {
        ...
        String resultat = objetService.acheter(999L, 999L);
